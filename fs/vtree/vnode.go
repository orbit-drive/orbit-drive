...
type VNode struct {
	...
	// Name holds the relative path to the head vnode.
...
func (vn *VNode) GetAbsolutePath() string {
	if vn.parent == nil {
		return config.GetRootPath()
	}
	parentPath := vn.parent.GetAbsolutePath()
	return filepath.Join(parentPath, vn.Name)
}
...
func (vn *VNode) SaveSource() error {
	...
	if !vn.IsNew() {
		s, err := ipfs.UploadFile(vn.GetAbsolutePath())
		if err != nil {
			return err
		}
		vn.Source.SetSrc(s)
...
func (vn *VNode) PopulateNodes(s db.Sources) error {
	...
	for _, f := range files {
		abspath := filepath.Join(absPath, f.Name())
...
func (vn *VNode) traverse(steps []string) (*VNode, error) {
	...
	for _, step := range steps {
		p := filepath.Join(vn.GetAbsolutePath(), step)
		i := vn.GenChildID(p)
